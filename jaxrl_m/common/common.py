...
def make_dict_args_hashable_decorator(func):
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        args = tuple(
            HashableDict(arg) if isinstance(arg, dict) else arg for arg in args
        )
        kwargs = {
...
def make_dict_kwargs_hashable_decorator(func):
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        kwargs = {
            k: HashableDict(v) if isinstance(v, dict) else v for k, v in kwargs.items()
