...
def make_optimizer(
    ...
    **kwargs,
    ...
    if cosine_decay_steps is not None and cosine_decay_steps > 0:
    ...
    def optimizer(learning_rate: float, weight_decay: Optional[float]):
        ...
        if weight_decay is not None:
            optimizer_stages.append(
                optax.adamw(
                    learning_rate=learning_rate, weight_decay=weight_decay, **kwargs
                )
            )
        else:
            optimizer_stages.append(optax.adam(learning_rate=learning_rate, **kwargs))
